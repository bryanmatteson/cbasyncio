...
from cbasyncio import fork as fork_mod
from cbasyncio.fork import PidfdChildProcess, _has_pidfd, afork
...
async def _do_test_fork():
    ...
    assert proc._pid > 0  # type: ignore
...
async def _do_test_fork_segfault():
    ...
    os.setpgrp()
    proc = await afork(child)  # type: ignore
    assert proc._pid > 0  # type: ignore
    if isinstance(proc, PidfdChildProcess):
        assert proc._pidfd > 0
    ret = await proc.wait()
...
@pytest.mark.skipif(not _has_pidfd, reason="pidfd is supported in Python 3.9+ and Linux kernel 5.4+")
@pytest.mark.asyncio
async def test_fork():
    await _do_test_fork()
...
async def test_fork_fallback():
    with mock.patch.object(fork_mod, "_has_pidfd", False):
        await _do_test_fork()
