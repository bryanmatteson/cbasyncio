...
from cbasyncio.aio import CancelScope, TaskGroup, fail_after, wait_all_tasks_blocked
from cbasyncio.errors import BrokenResourceError, ClosedResourceError, EndOfStream, WouldBlock
from cbasyncio.streamz import RecvChannel, SendChannel, create_channel_pair


def test_invalid_max_buffer() -> None:
    pytest.raises(ValueError, create_channel_pair, 1.0).match("max_buffer_size must be either an integer or math.inf")


def test_negative_max_buffer() -> None:
    pytest.raises(ValueError, create_channel_pair, -1).match("max_buffer_size cannot be negative")
...
async def test_receive_then_send() -> None:
    ...
    send, receive = create_channel_pair(0)
...
async def test_send_is_unblocked_after_receive_nowait() -> None:
    send, receive = create_channel_pair(1)
    send.send_nowait("hello")
...
async def test_send_nowait_then_receive_nowait() -> None:
    send, receive = create_channel_pair(2)
    send.send_nowait("hello")
    send.send_nowait("anyio")
    assert receive.receive_nowait() == "hello"
    assert receive.receive_nowait() == "anyio"
...
async def test_iterate() -> None:
    ...
    send, receive = create_channel_pair()
    received_objects: List[str] = []
    async with TaskGroup() as tg:
        tg.start_soon(receiver)
        await send.send("hello")
...
async def test_clone() -> None:
    send1, receive1 = create_channel_pair(1)
    send2 = send1.clone()
...
async def test_close_receive_after_send() -> None:
    ...
    send_stream, receive_stream = create_channel_pair()
    async with TaskGroup() as tg:
        tg.start_soon(send)
...
async def test_statistics() -> None:
    send_stream, receive_stream = create_channel_pair(1)
    streams: List[Union[RecvChannel[int], SendChannel[int]]] = [send_stream, receive_stream]
